...
export function getTree(): Promise<BookmarkNode[]> {
  ...
  return callChrome<BookmarkNode[]>((cb) => chrome.bookmarks.getTree(cb)).then(
...
export function getChildren(folderId: string): Promise<BookmarkNode[]> {
  ...
  return callChrome<BookmarkNode[]>((cb) => chrome.bookmarks.getChildren(folderId, cb)).then(
    (nodes) => nodes as unknown as BookmarkNode[],
  )
}
...
export function updateBookmark(
  ...
  return callChrome<BookmarkNode>((cb) => chrome.bookmarks.update(id, changes, cb)).then(
...
export function removeBookmark(id: string, recursive = false): Promise<void> {
  ...
  return callChrome<void>((cb) => invoke(id, cb))
}
...
export function moveBookmark(
  ...
  if (!isExtensionEnv()) return Promise.reject(new Error('not in extension env'))
  return callChrome<BookmarkNode>((cb) => chrome.bookmarks.move(id, destination, cb)).then(
    (node) => node as unknown as BookmarkNode,
  )
}
